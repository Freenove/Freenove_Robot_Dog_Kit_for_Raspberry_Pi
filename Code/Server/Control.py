...
class Control:
    def __init__(self):
        ...
        self.move_timeout = 0
        self.order = ['','','','','']
        self.point = [[0, 99, 0], [0, 99, 0], [0, 99, 0], [0, 99, 0]]
                ...
                if self.move_count < 180:
                    ...
                    if self.attitude_flag==True and self.order[0] !=cmd.CMD_ATTITUDE and self.order[0] != '':
                        if self.attitude_flag==True and self.order[0] !=cmd.CMD_ATTITUDE and self.order[0] != '':
                           self.stop()   
                           self.attitude_flag=False  
                    if self.relax_flag==False and self.order[0] != '':
    ...
    def changeCoordinates(self,move_order,X1=0,Y1=96,Z1=0,X2=0,Y2=96,Z2=0,pos=np.mat(np.zeros((3, 4)))):
        if move_order == 'turnLeft':  
            for i in range(2):
                self.point[2*i][0]=((-1)**(1+i))*X1*2
                self.point[2*i][1]=Y1
                self.point[2*i][2]=((-1)**(i))*Z1
                self.point[1+2*i][0]=((-1)**(1+i))*X2
                self.point[1+2*i][1]=Y2
                self.point[1+2*i][2]=((-1)**(1+i))*Z2*2
        elif move_order == 'turnRight': 
            for i in range(2):
                self.point[2*i][0]=((-1)**(i))*X1
                self.point[2*i][1]=Y1
                self.point[2*i][2]=((-1)**(1+i))*Z1*2
                self.point[1+2*i][0]=((-1)**(i))*X2*2
                self.point[1+2*i][1]=Y2
                self.point[1+2*i][2]=((-1)**(i))*Z2
            ...
            for i in range(2):
                ...
                self.point[i*2][1]=Y1
                self.point[i*2][2]=Z1
                self.point[i*2+1][0]=X2
                self.point[i*2+1][1]=Y2
                self.point[i*2+1][2]=Z2
        self.run()
    def backWard(self):
        for i in range(450,89,-self.speed):
            X1=10*math.cos(i*math.pi/180)
            Y1=8*math.sin(i*math.pi/180)+self.height
            X2=10*math.cos((i+180)*math.pi/180)
    ...
    def turnLeft(self):
        for i in range(0,361,self.speed):
            X1=5*math.cos(i*math.pi/180)
            Y1=10*math.sin(i*math.pi/180)+self.height
            X2=5*math.cos((i+180)*math.pi/180)
            Y2=10*math.sin((i+180)*math.pi/180)+self.height
            if Y2 > self.height:
                Y2=self.height
            if Y1 > self.height:
                Y1=self.height
    ...
    def stop(self):
        p=[[0, 0, 0], [10, 0, 0], [10, 0, 0], [0, 0, 0]]
        for i in range(4):
            p[i][0]=(0-self.point[i][0])/50
            p[i][1]=(self.height-self.point[i][1])/50
            p[i][2]=(0-self.point[i][2])/50
        for j in range(50):
            for i in range(4):
    ...
    def attitude(self,r,p,y):
        r=self.restriction(int(r),-20,20)
        p=self.restriction(int(p),-20,20)
        y=self.restriction(int(y),-20,20)
    ...
    def IMU6050(self):
        ...
        while True:
            ...
            if  (self.order[0]==cmd.CMD_BALANCE and self.order[1]=='0')or(self.balance_flag==True and self.order[0]!='')or(self.move_count>180):
                ...
                break
    def check(self,pos):
        number0=math.sqrt(pos[0,0]**2+pos[1,0]**2+pos[2,0]**2)
        number1=math.sqrt(pos[0,1]**2+pos[1,1]**2+pos[2,1]**2)
        number2=math.sqrt(pos[0,2]**2+pos[1,2]**2+pos[2,2]**2)
        number3=math.sqrt(pos[0,3]**2+pos[1,3]**2+pos[2,3]**2)
        if number0 > 133 or number1 > 133 or number2 > 133 or number3 > 133:
            return False
        return True
    ...
    def postureBalance(self,r,p,y,h=1):
        ...
        body_struc = np.mat([[ l / 2,  b / 2,  0],
                           ...
                           [-l / 2,  b / 2,    0],
                           [-l / 2, -b / 2,    0]]).T
        footpoint_struc = np.mat([[ l / 2,  w / 2,  self.height-h],
                                [ l / 2, -w / 2,    self.height-h],
                                [-l / 2,  w / 2,    self.height-h],
                                [-l / 2, -w / 2,    self.height-h]]).T
